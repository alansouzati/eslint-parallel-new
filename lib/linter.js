...
function getIgnorePatterns(options) {
  ...
  try {
    const ignore = fs.readFileSync(ignorePath, 'utf8').trim().split('\n');
...
function hasEslintCache(options) {
  const cacheLocation = options.cacheFile || options.cacheLocation || path.join(options.cwd || process.cwd(), '.eslintcache');
  try {
...
function getFilesByPatterns(patterns, options) {
  ...
  patterns.forEach(pattern => {
    ...
    if (isFile) {
      ...
      const path = isFolder ? `${ pattern.replace(/\'|\"|\/$/g, '') }/**` : pattern.replace(/\'|\"|\/$/g, '');
      const newFiles = glob.sync(path, { cwd: options.cwd || process.cwd(), nodir: true, ignore });
...
export default class Linter {
  ...
  execute(patterns) {
    return new Promise((resolve, reject) => {
      const files = getFilesByPatterns(patterns, this._options);
